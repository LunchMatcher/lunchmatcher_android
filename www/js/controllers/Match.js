...
.controller('matchCtrl', ['$scope', '$timeout', '$stateParams', '$http', '$ionicLoading', 'global', 'config', '$ionicPopup', 'matches', '$http', '$location', '$q','$ionicViewService',function($scope, $timeout, $stateParams, $http, $ionicLoading, global, config, $ionicPopup, matches, $http, $location, $q,$ionicViewService) {
			...
			$scope.matchedUser 		= {
				...
				$http({
					    ...
					    if(response.status == 'ok'){
							...
							response.result.headline 		= response.result.headline;
...
// ***********matchRejectedFn
		$scope.matchRejectedFn = function(){
			var confirm = global.confirm('Are you sure to reject this match?');
