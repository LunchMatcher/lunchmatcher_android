						...
						if(share_type == 'facebook'){
							$http.post(config.apiUrl+'/getShare', { 'ref_id' : $stateParams.logid, 'point_id': 9, 'member_id' : config.userProfile.member_id }).   //temp code 
								...
								if(response.status == 'success'){
									...
									window.plugins.socialsharing.shareViaFacebookWithPasteMessageHint($scope.shareDesc, null , $scope.shareURL ,'Press and hold in the text area to paste prefilled text for your status update!'
									, function() {
									...
									global.popup('Thank you, You have already shared this Lunch status.');	
								}
