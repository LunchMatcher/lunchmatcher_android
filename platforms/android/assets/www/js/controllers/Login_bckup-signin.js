...
.controller('LoginCtrl',['$scope', '$ionicPopup', '$cordovaOauth', '$http','config', '$location', '$ionicLoading', 'global','$cordovaDatePicker','$ionicViewService', function($scope, $ionicPopup, $cordovaOauth, $http, config, $location, $ionicLoading, global,$cordovaDatePicker,$ionicViewService) {
		...
		$scope.user = { gender: '', phoneNumber : '', excludePreviousMatches : 'N' , get_notification: '1' , notificationTime: '09:30 AM'};
		...
		$scope.companyTemp = [{}];
		$scope.page = 10;			  
  		$scope.url		='https://api.linkedin.com/v1/people/~:(id,first-name,last-name,maiden-name,formatted-name,site-standard-profile-request,industry,picture-url,picture-urls::(original),headline,email-address,location:(name),positions,date-of-birth)?oauth2_access_token=' ;
...
// ***********getProfile
	  ...
	  $scope.signIn = function(form) {
           ...
           $http.post(config.clientUrl + '/login', {
                'username': $scope.authorization.username,
                'password': $scope.authorization.password
            }).success(function(data){
				console.log(data);
                if (data.status == 'ok') {
                    localStorage.setItem("lmUserProfile", JSON.stringify(data.result));
					$location.path("/app/availability");
                }
                else {
                   global.popup('Invalid email or password!');
					$location.path("/login");
                }
                ...
                $cordovaToast.show('An error occured.Please try again', 'short', 'center').then(function(success) {
